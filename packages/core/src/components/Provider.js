...
const Provider = ({ children, components }) => {
  ...
  const [isDarkMode, setIsDarkMode] = React.useState(stored === 'true');
  ...
  return (
    <ThemeProvider theme={isDarkMode ? dark : light}>
      <>
        <GlobalStyle />
        {children}
        <p style={{ color: 'red' }}>{isDarkMode ? 'Enabled' : 'Disabled'}</p>
        <Button
          onClick={() => {
            setIsDarkMode(isDarkMode === false);
            localStorage.setItem('isDarkMode', isDarkMode === false);
          }}
        >
          Toggle Dark Mode
        </Button>
